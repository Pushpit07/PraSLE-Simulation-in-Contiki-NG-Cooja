#!/bin/bash
#
# Bully Algorithm Network Partition Trials
#
# Runs multiple trials to collect convergence time statistics under network
# partition conditions (two isolated node groups).
#
...
#
# Usage:
#   ./run_partition_trials.sh <node_count> [trials] [duration] [parallel_jobs] [--fixed-seed]
#
# Parameters:
...
#
# Examples:
#   ./run_partition_trials.sh 50              # 100 trials, 50 nodes
#   ./run_partition_trials.sh 100 200         # 200 trials, 100 nodes
#   ./run_partition_trials.sh 10 50 90        # 50 trials, 10 nodes, 90s each
#   ./run_partition_trials.sh 50 100 60 4     # 100 trials with 4 parallel jobs
#   ./run_partition_trials.sh 50 --fixed-seed # 100 trials with fixed seed (reproducible)
#
...
if [ ${#ARGS[@]} -lt 1 ]; then
    ...
    echo ""
    echo "Usage: $0 <node_count> [trials] [duration] [parallel_jobs] [--fixed-seed]"
    echo ""
    echo "Parameters:"
    ...
    echo ""
    echo "Examples:"
    echo "  $0 50              # 100 trials, 50 nodes"
    echo "  $0 100 200         # 200 trials, 100 nodes"
...
DURATION=${ARGS[2]:-60}
PARALLEL_JOBS=${ARGS[3]:-0}  # 0 means auto-detect
...
SIMULATION_FILE="$CSC_TEMPLATES_DIR/${NODE_COUNT}nodes-partition.csc"
...
OUTPUT_BASE="$BULLY_DIR/results/network_partition/${NODE_COUNT}nodes_partition_$(date +%Y%m%d_%H%M%S)"
mkdir -p "$OUTPUT_BASE"
...
echo "=============================================================="
echo " Bully Algorithm Network Partition Trials"
echo "=============================================================="
...
# Create CSV header
echo "trial,node_count,partition_a_convergence_ms,partition_b_convergence_ms,partition_a_leader,partition_b_leader,split_brain" > "$CSV_FILE"
...
run_trial() {
    ...
    # Extract partition-specific metrics from metrics.csv
    ...
    if [ -f "$trial_output/metrics.csv" ]; then
        ...
        read partition_a_convergence partition_b_convergence partition_a_leader partition_b_leader split_brain < <(python3 -c "
import csv
import sys

partition_a_min = $PARTITION_A_MIN
partition_a_max = $PARTITION_A_MAX
partition_b_min = $PARTITION_B_MIN
partition_b_max = $PARTITION_B_MAX

partition_a_times = []
partition_b_times = []
partition_a_leaders = set()
partition_b_leaders = set()
...
try:
    with open('$trial_output/metrics.csv', 'r') as f:
        ...
        for row in reader:
            ...
            conv_time = row.get('first_convergence_time', '0')
            if conv_time and float(conv_time) > 0:
                if partition_a_min <= node_id <= partition_a_max:
                    partition_a_times.append(float(conv_time))
                elif partition_b_min <= node_id <= partition_b_max:
                    partition_b_times.append(float(conv_time))

            # Get current leader reported by each node
            leader = row.get('current_leader', '0')
            if leader and int(leader) > 0:
                if partition_a_min <= node_id <= partition_a_max:
                    partition_a_leaders.add(int(leader))
                elif partition_b_min <= node_id <= partition_b_max:
                    partition_b_leaders.add(int(leader))

    # Calculate results
    a_conv = int(min(partition_a_times)) if partition_a_times else 'NA'
    b_conv = int(min(partition_b_times)) if partition_b_times else 'NA'

    # Determine the final leader for each partition (most common, or expected highest ID)
    a_leader = max(partition_a_leaders) if partition_a_leaders else 'NA'
    b_leader = max(partition_b_leaders) if partition_b_leaders else 'NA'

    # Check for split-brain (two different leaders in two partitions)
    split_brain = 'true' if (a_leader != 'NA' and b_leader != 'NA' and a_leader != b_leader) else 'false'

    print(f'{a_conv} {b_conv} {a_leader} {b_leader} {split_brain}')

except Exception as e:
    print('NA NA NA NA false', file=sys.stderr)
    print('NA NA NA NA false')
" 2>/dev/null)
    ...
    echo "$trial,$NODE_COUNT,$partition_a_convergence,$partition_b_convergence,$partition_a_leader,$partition_b_leader,$split_brain" > "$OUTPUT_BASE/.trial_${trial}.result"
}

# Export function and variables for parallel execution
export -f run_trial
export OUTPUT_BASE SIMULATION_FILE SCRIPTS_DIR DURATION NODE_COUNT USE_RANDOM_SEED
...
for trial in $(seq 1 $NUM_TRIALS); do
    if [ -f "$OUTPUT_BASE/.trial_${trial}.result" ]; then
        ...
        echo "$trial,$NODE_COUNT,NA,NA,NA,NA,false" >> "$CSV_FILE"
    fi
done
...
with open('$CSV_FILE', 'r') as f:
    ...
    for row in reader:
        ...
        if row['partition_a_convergence_ms'] != 'NA':
            partition_a_times.append(float(row['partition_a_convergence_ms']))
        if row['partition_b_convergence_ms'] != 'NA':
            partition_b_times.append(float(row['partition_b_convergence_ms']))
