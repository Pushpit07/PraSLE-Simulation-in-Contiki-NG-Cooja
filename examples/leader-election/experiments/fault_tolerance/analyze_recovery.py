#!/usr/bin/env python3
"""
Leader Election Algorithm Crash Recovery Analysis
...
Usage:
    python3 analyze_recovery.py -i <csv_file> [-o <output_dir>] [-a <algorithm>]
...
Parameters:
    ...
    -a, --algorithm  Algorithm name for labeling (default: auto-detect from CSV)
...
def load_recovery_data(csv_file):
    ...
    Args:
        csv_file: Path to CSV file with columns:
                  trial, [algorithm,] node_count, crash_time_s, initial_convergence_time_ms,
                  recovery_convergence_time_ms, total_recovery_time_ms

    Returns:
        tuple: (trials, algorithm, node_count, crash_time, initial_times, recovery_times, total_recovery_times)
    """
    trials = []
    initial_times = []
    recovery_times = []
    ...
    with open(csv_file, 'r') as f:
        ...
        for row in reader:
            ...
            # Check if algorithm column exists
            if 'algorithm' in row:
    ...
    return trials, algorithm, node_count, crash_time, initial_times, recovery_times, total_recovery_times
...
def print_statistics(initial_stats, recovery_stats, algorithm, node_count, crash_time):
    """
    ...
    """
    algo_str = f" ({algorithm.upper()})" if algorithm else ""
    print(f"\nLeader Crash Recovery Analysis{algo_str}:")
    print("=" * 70)
    print(f"Algorithm:         {algorithm or 'unknown'}")
...
def plot_recovery_timeline(trials, initial_times, recovery_times, total_recovery_times,
                           ...
                           algorithm, node_count, output_file, dpi=300):
    """
    ...
    # Configure plot
    algo_str = f" - {algorithm.upper()}" if algorithm else ""
    plt.title(f'Leader Crash Recovery Timeline{algo_str} - {node_count} Nodes ({len(valid_trials)} trials)',
...
def plot_recovery_distribution(trials, total_recovery_times, stats_recovery,
                                algorithm, node_count, crash_time, output_file, dpi=300):
    """
    ...
    # Configure plot
    algo_str = f" - {algorithm.upper()}" if algorithm else ""
    plt.title(f'Recovery Time Distribution{algo_str} - {node_count} Nodes (Crash at {crash_time}s)',
              fontsize=14, fontweight='bold', pad=20)
    plt.xlabel('Trial Number', fontsize=12)
    ...
    algo_label = f'{algorithm.upper()}\n' if algorithm else ''
    textstr = '\n'.join((
        f'{algo_label}Recovery Time Statistics:',
...
def save_summary_csv(output_dir, initial_stats, recovery_stats, algorithm, node_count, crash_time):
    """
    ...
    with open(summary_file, 'w', newline='') as f:
        writer = csv.writer(f)
        writer.writerow(['Metric', 'Algorithm', 'Initial_Convergence_ms', 'Recovery_Time_ms'])
        writer.writerow(['Node_Count', algorithm or '', node_count, node_count])
        writer.writerow(['Crash_Time_s', algorithm or '', '-', crash_time])
        writer.writerow(['Valid_Trials', algorithm or '', initial_stats['valid_count'], recovery_stats['valid_count']])
        writer.writerow(['Mean', algorithm or '', f"{initial_stats['mean']:.2f}", f"{recovery_stats['mean']:.2f}"])
        writer.writerow(['Median', algorithm or '', f"{initial_stats['median']:.2f}", f"{recovery_stats['median']:.2f}"])
        writer.writerow(['Std_Dev', algorithm or '', f"{initial_stats['stdev']:.2f}", f"{recovery_stats['stdev']:.2f}"])
        writer.writerow(['Min', algorithm or '', f"{initial_stats['min']:.2f}", f"{recovery_stats['min']:.2f}"])
        writer.writerow(['Max', algorithm or '', f"{initial_stats['max']:.2f}", f"{recovery_stats['max']:.2f}"])
        writer.writerow(['CV_%', algorithm or '', f"{initial_stats['cv']:.2f}", f"{recovery_stats['cv']:.2f}"])
...
def main():
    ...
    parser = argparse.ArgumentParser(
        ...
        epilog="""
Examples:
  %(prog)s -i results/bully/fault_tolerance/50nodes_crash60s/crash_recovery_times.csv
  %(prog)s -i data.csv -o analysis_output/ --algorithm ring
  %(prog)s -i data.csv --no-plots
    ...
    parser.add_argument('-a', '--algorithm',
                        ...
                        default=None,
                        help='Algorithm name for labeling (default: auto-detect from CSV)')
    ...
    try:
        ...
        trials, csv_algorithm, node_count, crash_time, initial_times, recovery_times, total_recovery_times = \
            load_recovery_data(args.input)

        # Use algorithm from args or from CSV
        algorithm = args.algorithm or csv_algorithm
        ...
        print_statistics(stats_initial, stats_recovery, algorithm, node_count, crash_time)

        # Save summary CSV
        save_summary_csv(output_dir, stats_initial, stats_recovery, algorithm, node_count, crash_time)
        ...
        if not args.no_plots:
            ...
            # Timeline plot
            timeline_file = output_dir / f'recovery_timeline{algo_suffix}_{node_count}nodes.png'
            plot_recovery_timeline(trials, initial_times, recovery_times, total_recovery_times,
                                   crash_time, stats_initial, stats_recovery,
                                   algorithm, node_count, timeline_file, args.dpi)

            # Distribution plot
            dist_file = output_dir / f'recovery_distribution{algo_suffix}_{node_count}nodes.png'
            plot_recovery_distribution(trials, total_recovery_times, stats_recovery,
                                       algorithm, node_count, crash_time, dist_file, args.dpi)
