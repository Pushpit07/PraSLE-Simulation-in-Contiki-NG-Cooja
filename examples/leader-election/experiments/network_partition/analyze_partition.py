#!/usr/bin/env python3
"""
Leader Election Algorithm Network Partition Analysis Script
...
Usage:
    python3 analyze_partition.py -i partition_times.csv [-o output_dir] [-a algorithm] [--no-plots]

Input CSV format:
    trial,algorithm,node_count,partition_a_convergence_ms,partition_b_convergence_ms,
...
def load_data(csv_path):
    ...
    with open(csv_path, 'r') as f:
        ...
        for row in reader:
            ...
            # Get algorithm if present
            if data['algorithm'] is None and 'algorithm' in row:
...
def print_report(data, config, algorithm=None):
    """Print detailed analysis report to console."""
    total_trials = len(data['trials'])
    node_count = data['node_count']

    # Use algorithm from data if not provided
    algo = algorithm or data['algorithm'] or 'unknown'
    algo_upper = algo.upper()

    print()
    print(f"Network Partition Analysis - {algo_upper}:")
    print("=" * 70)
...
def save_summary_csv(data, config, a_stats, b_stats, output_path, algorithm=None):
    ...
    with open(output_path, 'w', newline='') as f:
        ...
        writer.writerow(['algorithm', algo, algo])
...
def create_convergence_plot(data, config, a_stats, b_stats, output_path, algorithm=None):
    ...
    ax1.set_title(f'{algo_upper} - Network Partition Convergence Times\n({node_count} nodes)', fontsize=14)
    ...
    ax2.set_title(f'{algo_upper} - Convergence Time Distribution\n({node_count} nodes)', fontsize=14)
...
def create_leader_plot(data, config, output_path, algorithm=None):
    ...
    ax1.set_title(f'{algo_upper} - Partition A Leader Distribution\n(expected: Node {config["a_expected"]})', fontsize=12)
    ...
    ax2.set_title(f'{algo_upper} - Partition B Leader Distribution\n(expected: Node {config["b_expected"]})', fontsize=12)
    ax2.grid(True, alpha=0.3, axis='y')
...
def main():
    parser = argparse.ArgumentParser(
        description='Analyze leader election algorithm network partition trial results'
    )
    ...
    parser.add_argument('-a', '--algorithm',
                       ...
                       default=None,
                       help='Algorithm type (default: auto-detect from CSV)')
    ...
    algorithm = args.algorithm or data['algorithm']
    ...
    a_stats, b_stats = print_report(data, config, algorithm)

    # Save summary CSV
    summary_path = os.path.join(output_dir, 'partition_summary.csv')
    save_summary_csv(data, config, a_stats, b_stats, summary_path, algorithm)
    ...
    if not args.no_plots and HAS_MATPLOTLIB:
        ...
        conv_plot_path = os.path.join(output_dir, f'partition_convergence_{node_count}nodes{algo_suffix}.png')
        create_convergence_plot(data, config, a_stats, b_stats, conv_plot_path, algorithm)

        # Leader distribution plot
        leader_plot_path = os.path.join(output_dir, f'partition_leaders_{node_count}nodes{algo_suffix}.png')
        create_leader_plot(data, config, leader_plot_path, algorithm)
