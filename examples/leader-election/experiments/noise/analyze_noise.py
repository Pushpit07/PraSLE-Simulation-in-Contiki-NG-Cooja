#!/usr/bin/env python3
"""
Leader Election Algorithm Network Disruption Analysis
...
Usage:
    python3 analyze_noise.py -i <csv_file> [-o <output_dir>] [-a <algorithm>]
...
def load_noise_data(csv_file):
    ...
    Args:
        csv_file: Path to CSV file with columns:
                  trial, [algorithm,] node_count, noise_level, convergence_time_ms, message_count

    Returns:
        tuple: (trials, algorithm, node_count, noise_level, convergence_times, message_counts)
    """
    trials = []
    convergence_times = []
    message_counts = []
    ...
    return trials, algorithm, node_count, noise_level, convergence_times, message_counts
...
def print_statistics(conv_stats, msg_stats, algorithm, node_count, noise_level):
    """
    ...
    print(f"\nNetwork Disruption Analysis{algo_str}:")
    print("=" * 70)
...
def plot_convergence_distribution(trials, convergence_times, conv_stats,
                                   algorithm, node_count, noise_level, output_file, dpi=300):
    """
    ...
    # Configure plot
    algo_str = f" - {algorithm.upper()}" if algorithm else ""
    plt.title(f'Convergence Time Distribution{algo_str} - {node_count} Nodes ({noise_level}% Success Rate)',
    ...
    textstr = '\n'.join((
        f'{algo_label}Convergence Statistics:',
...
def plot_message_overhead(trials, message_counts, msg_stats,
    ...
    # Configure plot
    algo_str = f" - {algorithm.upper()}" if algorithm else ""
    plt.title(f'Message Overhead{algo_str} - {node_count} Nodes ({noise_level}% Success Rate)',
              fontsize=14, fontweight='bold', pad=20)
    plt.xlabel('Trial Number', fontsize=12)
...
def save_summary_csv(output_dir, conv_stats, msg_stats, algorithm, node_count, noise_level):
    """
    ...
    with open(summary_file, 'w', newline='') as f:
        writer = csv.writer(f)
        writer.writerow(['Metric', 'Algorithm', 'Convergence_Time_ms', 'Message_Count'])
        writer.writerow(['Node_Count', algorithm or '', node_count, node_count])
        writer.writerow(['Noise_Level_%', algorithm or '', noise_level, noise_level])
        writer.writerow(['Packet_Loss_%', algorithm or '', 100-noise_level, 100-noise_level])
        writer.writerow(['Valid_Trials', algorithm or '', conv_stats['valid_count'], msg_stats['valid_count']])
        writer.writerow(['Mean', algorithm or '', f"{conv_stats['mean']:.2f}", f"{msg_stats['mean']:.0f}"])
        writer.writerow(['Median', algorithm or '', f"{conv_stats['median']:.2f}", f"{msg_stats['median']:.0f}"])
        writer.writerow(['Std_Dev', algorithm or '', f"{conv_stats['stdev']:.2f}", f"{msg_stats['stdev']:.0f}"])
        writer.writerow(['Min', algorithm or '', f"{conv_stats['min']:.2f}", f"{msg_stats['min']:.0f}"])
        writer.writerow(['Max', algorithm or '', f"{conv_stats['max']:.2f}", f"{msg_stats['max']:.0f}"])
        writer.writerow(['CV_%', algorithm or '', f"{conv_stats['cv']:.2f}", f"{msg_stats['cv']:.2f}"])
...
def main():
    ...
    parser = argparse.ArgumentParser(
        ...
        epilog="""
Examples:
  %(prog)s -i results/bully/noise/50nodes_noise90/noise_convergence_times.csv
  ...
  %(prog)s -i data.csv --no-plots
    ...
    try:
        ...
        trials, csv_algorithm, node_count, noise_level, convergence_times, message_counts = \
        ...
        print_statistics(conv_stats, msg_stats, algorithm, node_count, noise_level)

        # Save summary CSV
        save_summary_csv(output_dir, conv_stats, msg_stats, algorithm, node_count, noise_level)
        ...
        if not args.no_plots:
            ...
            conv_file = output_dir / f'noise{noise_level}_convergence{algo_suffix}_{node_count}nodes.png'
            plot_convergence_distribution(trials, convergence_times, conv_stats,
                                          algorithm, node_count, noise_level, conv_file, args.dpi)

            # Message overhead plot
            msg_file = output_dir / f'noise{noise_level}_messages{algo_suffix}_{node_count}nodes.png'
            plot_message_overhead(trials, message_counts, msg_stats,
                                  algorithm, node_count, noise_level, msg_file, args.dpi)
