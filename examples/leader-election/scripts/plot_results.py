#!/usr/bin/env python3
"""
Leader Election Algorithm Results Visualization

This script creates visualizations from leader election algorithm metrics.
Supports all algorithms: bully, ring, prasle, adaptive-prasle.

Usage:
    python3 plot_results.py metrics.csv --output plots/
    python3 plot_results.py metrics.csv --algorithm ring --output plots/
...
class ResultsVisualizer:
    """Creates visualizations from leader election algorithm metrics"""
    ...
    def __init__(self, metrics_file, algorithm='bully'):
        ...
        print(f"Loaded {len(self.df)} metrics entries for {len(self.nodes)} nodes ({algorithm})")
    ...
    def plot_convergence_time(self, output_file):
        ...
        ax.set_title(f'Time to First Leader Election ({self.algorithm.upper()})')
    ...
    def plot_message_overhead(self, output_file):
        ...
        if self.algorithm == 'bully':
            ...
            # Generic message counting for other algorithms
            if 'messages_sent' in final_state.columns:
                df_sent = pd.DataFrame({'Messages': final_state['messages_sent']}, index=final_state.index)
            else:
                print("[WARNING] No message data available for this algorithm")
                return
        ...
        ax.set_title(f'Message Overhead per Node ({self.algorithm.upper()})')
    ...
    def plot_election_statistics(self, output_file):
        ...
        election_data = pd.DataFrame({
            'Started': final_state.get('elections_started', 0),
            'Won': final_state.get('elections_won', 0),
            'Lost': final_state.get('elections_lost', 0)
        ...
        ax.set_title(f'Election Statistics per Node ({self.algorithm.upper()})')
        ax.legend()
        ax.grid(axis='y', alpha=0.3)
        plt.tight_layout()
        plt.savefig(output_file, dpi=300)
    ...
    def plot_state_distribution(self, output_file):
        ...
        state_data = pd.DataFrame({
            'NORMAL': final_state.get('time_in_normal', 0),
            'ELECTION': final_state.get('time_in_election', 0),
            'WAITING': final_state.get('time_in_waiting', 0)
        }, index=final_state.index)
        ...
        ax.set_title(f'State Distribution per Node ({self.algorithm.upper()})')
    ...
    def plot_leader_timeline(self, output_file):
        ...
        for node_id in sorted(self.nodes):
            ...
            if 'current_leader' in node_data.columns:
        ...
        ax.set_title(f'Leader Convergence Timeline ({self.algorithm.upper()})')
    ...
    def plot_message_timeline(self, output_file):
        ...
        for node_id in sorted(self.nodes):
            ...
            elif 'messages_sent' in node_data.columns:
                total_sent = node_data['messages_sent']
            else:
                continue
        ...
        ax1.set_title(f'Message Transmission Timeline ({self.algorithm.upper()})')
        ax1.legend()
        ...
        for node_id in sorted(self.nodes):
            node_data = self.df[self.df['node_id'] == node_id]
            if 'elections_started' in node_data.columns:
...
def main():
    parser = argparse.ArgumentParser(
        description="Visualize leader election algorithm metrics",
    ...
    try:
        visualizer = ResultsVisualizer(args.metrics_file, args.algorithm)
