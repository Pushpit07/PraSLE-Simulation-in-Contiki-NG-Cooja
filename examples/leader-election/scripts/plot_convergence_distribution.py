#!/usr/bin/env python3
"""
Leader Election Algorithm Convergence Time Distribution Plotter
...
Usage:
    ...
    python3 plot_convergence_distribution.py -i <csv_file> --algorithm ring -o <output_png>

Parameters:
    -i, --input      Path to convergence_times.csv file (required)
    -o, --output     Output PNG file path (default: convergence_distribution.png)
    -a, --algorithm  Algorithm type (default: auto-detect from CSV)
...
def load_convergence_data(csv_file):
    ...
    Args:
        csv_file: Path to CSV file with columns: trial, [algorithm,] node_count, convergence_time_ms

    Returns:
        tuple: (trials, node_count, convergence_times, algorithm)
    """
    trials = []
    convergence_times = []
    node_count = None
    ...
    return trials, node_count, convergence_times, algorithm
...
def print_statistics(stats, node_count, algorithm=None):
    """
    ...
    print(f"\nConvergence Time Statistics{algo_str}:")
    print("=" * 60)
...
def plot_convergence_distribution(trials, convergence_times, stats, node_count,
                                   output_file, algorithm=None, title=None, dpi=300):
    """
    ...
    if title:
        ...
        algo_str = f" - {algorithm.upper()}" if algorithm else ""
        plt.title(f'Convergence Time Distribution{algo_str} - {node_count} Nodes ({stats["count"]} trials)',
    ...
    textstr = '\n'.join((
        f'{algo_label}Mean = {mean_val:.1f} ms',
...
def main():
    ...
    parser = argparse.ArgumentParser(
        ...
        epilog="""
Examples:
  %(prog)s -i results/bully/convergence_trials/50nodes_20251201/convergence_times.csv
  %(prog)s -i data.csv --algorithm ring -o output.png
  %(prog)s -i data.csv --title "Custom Title" --dpi 150
        """
    )
    ...
    try:
        ...
        trials, node_count, convergence_times, csv_algorithm = load_convergence_data(args.input)
        ...
        print_statistics(stats, node_count, algorithm)

        # Generate and save plot
        plot_convergence_distribution(trials, convergence_times, stats, node_count,
                                       args.output, algorithm, args.title, args.dpi)
